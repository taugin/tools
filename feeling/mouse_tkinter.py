...
class ScreenTranform:
    ...
    def process_position(self, event):
        if (event.code == self.getpositiony() and getoriention() != "portrait"):
            tmp = int(event.value)
            tmp = ScreenTranform.HEIGHT-tmp
            event.value = "%d" % tmp
...
def mousenevent(event):
    ...
    if (pressing):
        phoneevent = PhoneEvent(EV_TOUCH_DEVICE, EV_ABS, screenTransform.getpositionx(), "%d" % event.x)
        sendevent(phoneevent)
        phoneevent = PhoneEvent(EV_TOUCH_DEVICE, EV_ABS, screenTransform.getpositiony(), "%d" % event.y)
        sendevent(phoneevent)

        phoneevent = PhoneEvent(EV_TOUCH_DEVICE, EV_ABS, ABS_MT_PRESSURE, "%d" % 35)
        sendevent(phoneevent)
...
def mouse_left_down(event):
    ...
    phoneevent = PhoneEvent(EV_TOUCH_DEVICE, EV_ABS, ABS_MT_TOUCH_MAJOR, "%d" % 6)
    sendevent(phoneevent)
...
def mouse_left_up(event):
    ...
    pressing = False
    phoneevent = PhoneEvent(EV_TOUCH_DEVICE, EV_ABS, ABS_MT_TRACKING_ID, "%d" % 0xffffffff)
    sendevent(phoneevent)
